...
name, it uses \"unnamed-staff\" for that part of the filename."
   (let* ((inst-name (ly:context-property context 'instrumentName))
          (in-file (find (lambda (x) (equal? (string-take-right x 3) ".ly")) (command-line)))
        )
...
#(define (format-note engraver event)
   (let* ((origin (ly:input-file-line-char-column
          ...
          (bar (ly:context-property context 'currentBarNumber))
          (barPos (format-moment (ly:context-property context 'measurePosition)))
                 )
     (ly:debug (_ "bar: ~a ~a") bar barPos)
     (print-line engraver
                 (if (ly:pitch? pitch)
                     "note"
                     "type")
...
#(define (format-measure engraver event)
    (ly:debug (_ "bar: ~a") event)
    (print-line engraver "measure"
    (ly:event-property event 'currentBarNumber)
    )
)

#(define (format-alt engraver event)
    (print-line engraver "alternative"
        (ly:event-property event 'currentBarNumber)
    )
)

#(define (format-stream engraver event)
    (ly:debug (_ "stream: ~a") engraver)
    (print-line engraver "stream"
        (ly:event-property event 'currentBarNumber)
    )
)

#(define (format-time-sig engraver event)
    (let* ((tsf (ly:event-property event 'beat-structure)))
        ;; (ly:debug (_ "tsf: ~a") tsf)
        (print-line engraver "time-sig"
            (ly:event-property event 'numerator)
...
#(define event-listener-engraver
  (make-engraver
    (listeners
     ...
     ;;(measure-counter-event . format-measure)
     ;;(StreamEvent . format-stream)
     (alternative-event . format-alt)
     (tie-event . format-tie))))

#(define event-listener-engraver-2
  (make-engraver
    (listeners
     (time-signature-event . format-time-sig)
     (alternative-event . format-alt)
     )
     )
     )
...
\layout {
  ...
  \context {
      % added to get time signature
      \Score
      \consists #event-listener-engraver-2
  }
  %\context {
%  \Staff
%  \consists #event-listener-engraver
